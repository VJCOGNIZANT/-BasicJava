...
public class Chemistry {
  ...
  public static String spellWord(String word, String[] elements) {
    ...
    for(String atom: elements){
      if(word.contains(atom.toLowerCase())){
        output= output.replace(atom.toLowerCase(),atom);
        check= check.replace(atom.toLowerCase(),"");
      }
    }
    if(check.equals("") ||check.equals(null)){
      return output.toString();
    }else{
      return check.toString()+" cannot spell word";
    }
